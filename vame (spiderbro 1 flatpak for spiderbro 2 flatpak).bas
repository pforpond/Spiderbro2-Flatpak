...
LET consolelog$ = "/var/data/spiderbrodata/consolelog.txt": REM sets console log file location
...
IF _FILEEXISTS("/var/data/spiderbrodata/engine.ddf") THEN
    OPEN "/var/data/spiderbrodata/engine.ddf" FOR INPUT AS #1
    ...
    IF ros$ = "win" THEN
        ...
        IF _FILEEXISTS("/var/data/spiderbrodata/filelocwin.ddf") THEN
            OPEN "/var/data/spiderbrodata/filelocwin.ddf" FOR INPUT AS #1
    ...
    ELSE
        ...
        IF _FILEEXISTS("/var/data/spiderbrodata/filelocother.ddf") THEN
            OPEN "/var/data/spiderbrodata/filelocother.ddf" FOR INPUT AS #1
            INPUT #1, dloc$, mloc$, ploc$, floc$, sloc$, oloc$, scriptloc$, museloc$, sfxloc$, pocketloc$, uiloc$, tloc$, aloc$, menuloc$
            CLOSE #1
        ELSE
            ERROR 420
    ...
    END IF
    REM loads colours
    IF _FILEEXISTS("/var/data/spiderbrodata/colours.ddf") THEN
        OPEN "/var/data/spiderbrodata/colours.ddf" FOR INPUT AS #1
...
IF _FILEEXISTS(sloc$ + "savedata.ddf") THEN
    ...
    INPUT #1, mapno, currency, nope, posx, posy, direction, nope, nope, nope, pocketcarry
    ...
    CLOSE #1
    OPEN "/var/data/spiderbrodata/options.ddf" FOR INPUT AS #1
