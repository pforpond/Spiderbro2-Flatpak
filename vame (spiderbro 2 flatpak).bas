...
ELSE
	IF usersetup = 0 THEN
			...
			SHELL _HIDE "cp -R spiderbrodata $XDG_DATA_HOME"
			SHELL _HIDE "cp -R polydata2 $XDG_DATA_HOME"
...
IF _COMMANDCOUNT = 0 THEN
    IF ros$ = "win" THEN LET dloc$ = "data\": LET consolelog$ = "/var/data/data/consolelog.txt"
    IF ros$ = "lnx" OR ros$ = "mac" THEN LET dloc$ = "/var/data/data/": LET consolelog$ = "/var/data/data/consolelog.txt"
    RETURN
END IF
DO
...
IF ros$ = "mac" OR ros$ = "lnx" THEN SHELL _HIDE "cp " + sloc$ + "options.ddf /var/data/polydata2/options.ddf": SHELL _HIDE "cp " + sloc$ + "options.ddf /var/data/spiderbrodata/options.ddf"
...
REM runs app
IF ros$ = "lnx" THEN SHELL "./" + seperate2$ + "_linux"
...
IF ros$ = "mac" OR ros$ = "lnx" THEN SHELL _HIDE "rm /var/data/polydata2/options.ddf": SHELL _HIDE "rm /var/data/spiderbrodata/options.ddf"
